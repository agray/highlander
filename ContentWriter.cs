...
namespace pheonix {
    public class ContentWriter {
        ...
        public void writePagesOutputFile(string rootDir, 
                                           string contentOutputFile, 
                                           string configOutputFile) {
            PagesWriter = new StreamWriter(contentOutputFile);
            exclusions.populateExclusionLists();
            createContentFile(rootDir, configOutputFile);
            writeContentFile(PagesWriter);
        }

        public void writeGoToOutputFile(string GoToOutputFile) {
            GoToWriter = new StreamWriter(GoToOutputFile);
            createGoToFile();
            writeConfigFile(GoToWriter);
        }

        

        private void createGoToFile() {
            FileContentBuilder.EmitGoToFile();
        }
